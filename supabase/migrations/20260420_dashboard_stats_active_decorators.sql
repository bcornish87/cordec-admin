-- Decorators/Staff counts now restrict to active profiles, matching the
-- counts shown on the Users page. Previously included inactive users.
...
AS $function$
  SELECT json_build_object(
    ...
    'decorators', (SELECT count(DISTINCT ur.user_id) FROM user_roles ur JOIN profiles p ON p.user_id = ur.user_id WHERE ur.role = 'decorator' AND p.is_active = true),
    'staff', (SELECT count(DISTINCT ur.user_id) FROM user_roles ur JOIN profiles p ON p.user_id = ur.user_id WHERE ur.role != 'decorator' AND p.is_active = true),
